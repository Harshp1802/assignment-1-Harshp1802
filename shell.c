...
char **get_input(char *input){
    ...
    token = strtok(input, sep); // Using strtok() to split the input string to tokes
    while (token != NULL){
        command[count] = token; // Setting each pointer of the array to corresponding arguments
        count++;
